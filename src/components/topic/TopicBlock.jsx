...
export default function TopicBlock({ topic, editing = false }) {
  ...
  const { updateModeDocument, deleteTopic, renameTopic } = useNotes();
  const { mode } = useMode();
  ...
  // Editing here is driven by the category-wide toggle (editing prop) OR this card's own toggle.
  const isEditing = Boolean(user) && (editing || localEditing);

  // Topic actions live inside the ⋮ menu — always available to a signed-in user.
  const menuActions = [];
  if (user) {
    menuActions.push({
      label: isEditing ? 'View mode' : 'Edit notes',
      // Per-topic: edit only this card. The category header toggle edits all topics.
  ...
  return (
    ...
    >
      ...
      <TopicContent
        ...
        notesEditing={isEditing}
        onNotesEditingChange={setLocalEditing}
